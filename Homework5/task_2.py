...
def repeats(our_str):
    """
        Добавляет порядок вхождения к каждой букве в строке в формате _n.

        Параметры:
        -----------
        our_str : str
            Входная строка, в которой нужно подсчитать вхождение букв.

        Возвращает:
        ------------
        new_str
            Строка, в которой к каждому символу добавили сколько он встречался через _n.
    """

    letter_count = {}  # Словарь для подсчёта вхождений каждого символа
    result_parts = []  # Список для частей итоговой строки
    ...
    for letter in our_str:
        ...
        # Формируем часть строки: символ + _ + номер вхождения
        part = f"{letter}_{letter_count[letter]}"
